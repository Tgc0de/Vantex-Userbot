...
from vantexproject import LOGGER
...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("vantexproject").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("vantexproject").info(f"Invalid Git Command")
        ...
        LOGGER("vantexproject").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("vantexproject").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    try:
        ...
        await client.set_chat_photo(gruplog.id, photo="vantexproject/asoyy.jpg")
        if await in_heroku():
